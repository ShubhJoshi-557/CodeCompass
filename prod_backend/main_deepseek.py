...
#     code_type = Column(String, index=True)  # 'function', 'class', 'chunk'
#     name = Column(String, index=True)       # Function/class name
#     parameters = Column(String)             # Function parameters
#     language = Column(String, index=True)   # Programming language
...
#         lang = STRUCTURED_EXTENSIONS.get(file_ext)
        
#         with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
#             content = f.read()
            
#         structures = []
#         if lang and code_parser.parsers.get(lang):
#             # Parse using tree-sitter
#             structures = code_parser.parse_code_structures(content, lang)
        
#         if not structures:
#             # Fallback to smaller chunks for non-structured files
#             chunks = chunk_file(content, STRUCTURE_CHUNK_SIZE, STRUCTURE_OVERLAP)
#             structures = [{
#                 'type': 'chunk',
#                 'name': f"Chunk {i+1}",
#                 'parameters': '',
...
#             "language": lang or os.path.splitext(filename)[1][1:],
#             "content": s['content'],
...
# # Smaller chunk size for non-structured files
# STRUCTURE_CHUNK_SIZE = 50
# STRUCTURE_OVERLAP = 5
...
#         for lang in set(STRUCTURED_EXTENSIONS.values()):
#             try:
#                 parser = Parser()
#                 parser.set_language(get_language_lib(lang))
#                 self.parsers[lang] = parser
#                 self.load_queries(lang)
#             except Exception as e:
#                 print(f"Error loading parser for {lang}: {str(e)}")

#     def load_queries(self, lang):
#         try:
#             query_path = f"queries/{lang}/scopes.scm"
#             with open(query_path, "r") as f:
#                 self.queries[lang] = self.parsers[lang].query(f.read())
#         except Exception as e:
#             print(f"Error loading queries for {lang}: {str(e)}")

#     def parse_code_structures(self, content, language):
...
#                 if tag == 'function' or tag == 'class':
#                     start_line = node.start_point[0] + 1
#                     end_line = node.end_point[0] + 1
#                     name = self.get_node_name(node, content)
#                     parameters = self.get_node_parameters(node, content)
...
#     def get_node_name(self, node, content):
#         # Implementation varies per language - simplified example for Python/JS
#         name_node = node.child_by_field_name('name')
#         if name_node:
#             return content[name_node.start_byte:name_node.end_byte]
#         return "anonymous"

#     def get_node_parameters(self, node, content):
...
#     branch: str = None
#     folder: str = None
#     code_type: str = None  # 'function', 'class', 'chunk'
#     language: str = None
#     min_lines: int = None
#     max_lines: int = None
...
#                     continue
#                 if query.code_type and snippet.get('code_type') != query.code_type:
#                     continue
#                 if query.language and snippet.get('language') != query.language:
#                     continue
#                 if query.min_lines and (snippet['end_line'] - snippet['start_line']) < query.min_lines:
#                     continue
#                 if query.max_lines and (snippet['end_line'] - snippet['start_line']) > query.max_lines:
#                     continue
                    
#                 results.append({
#                     "filename": snippet["filename"],
#                     "filepath": snippet["filepath"],
#                     "type": snippet.get("code_type", "chunk"),
...
# # --------------------------
# # AI Endpoints Enhancements
# # --------------------------
# @app.post("/ai-explain")
# async def explain_code(payload: SearchQuerySnippet):
#     prompt = f"""Explain this {payload.language} code structure:
#     Name: {payload.name}

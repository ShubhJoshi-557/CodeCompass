...
def process_file(file_path, repo_path, repo_name):
    """Process a single file and return its data or None if invalid"""
    try:
        ...
        with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
            content = f.read(102400)  # Read first 100KB to handle large files
            return {
                "filename": filename,
                "filepath": os.path.relpath(file_path, repo_path),
                "content": content,
                "repo": repo_name
            }
...
def clone_and_index_repo(repo_owner: str, repo_name: str, task_id: str):
    """Optimized repository processing with batch operations and parallel processing"""
    ...
    try:
        ...
        # Collect valid files in parallel
        ...
        for i in tqdm(range(0, total_files, BATCH_SIZE), desc="Generating embeddings"):
        ...
        # Save index and cache once at the end
